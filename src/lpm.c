...
static int lpm_symlink(lua_State* L) {
  ...
  #else
    DWORD flags = SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE | ((GetFileAttributesW(lua_toutf16(L, luaL_checkstring(L, 1))) & FILE_ATTRIBUTE_DIRECTORY) ? SYMBOLIC_LINK_FLAG_DIRECTORY : 0);
            ...
            for (int i = 0; i < 2; ++i) {
              ...
              while (1) {
                ...
                if (CertVerifyTimeValidity(NULL, pCertContext->pCertInfo) != 0)
                  continue;
                if (CertGetIntendedKeyUsage(pCertContext->dwCertEncodingType, pCertContext->pCertInfo, keyUsage, sizeof(keyUsage)) && !(keyUsage[0] & CERT_KEY_CERT_SIGN_KEY_USAGE))
                  continue;

                // This bit pulled directly from curl.
                DWORD reqSize;
                int found = 0;
